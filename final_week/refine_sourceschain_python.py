...
def main():
    ...
    new_db_custom = FAISS.load_local('ALL_Broken_Python_CUSTOM', VertexAIEmbeddings())
    ...
    qa = RetrievalQAWithSourcesChain.from_chain_type(llm = VertexAI(temperature=0.5, max_output_tokens=200),
                                     chain_type = "refine",
    ...
    qa_custom = RetrievalQAWithSourcesChain.from_chain_type(llm = VertexAI(temperature=0.5, max_output_tokens=200),
                                     chain_type = "refine",
    ...
    while queries_on:
        ...
        if quality_analysis:
            for query in list_quality_analysis:
                ...
                resultado = qa({"question": query})
                print(resultado['answer'])
                print(resultado['source_documents'])
                print("\n\n\033[32m"+"#$#$#$#$#$ CUSTOM #$#$#$#$##$#$#"+"\033[m\n\n")
                resultado = qa_custom({"question": query})
                print(resultado['answer'])
                print(resultado['source_documents'])
