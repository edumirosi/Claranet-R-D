...
def get_folders_from_github_repo(token):
    url = f'https://api.github.com/repos/edurochasi/langchain' #repo url
    headers = {
...
def get_all_files_recursive(folders, token, unalista, boolean):
    ...
    while boolean:
        for folder in folders:
            if 'libs' not in folder['path'] or folder['path']=='libs/experimental': #condition that makes it stop
                #boolean = False
                print('skipping')
                #break
            else:
...
def get_source_chunks(files):
    ...
    source_chunks = []
    python_spliter = RecursiveCharacterTextSplitter.from_language(language=Language.PYTHON, chunk_size = 1024, chunk_overlap = 100)
...
def main():
    ...
    link = "https://github.com/edurochasi/Python"
    boolean = True
    ...
    new_db.save_local('ALL_LangChain_Files')
    print('db saved')
